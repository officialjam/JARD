"use client";
/**
 * Career Copilot — full build (Supabase-backed, multi-user version)
 * -------------------------------------------------------------
 ...
 *
 * Persistence: Supabase Postgres, one row per signed-in user
 * (see app/api/profile/route.js), gated by proxy.js + the Google
 * sign-in flow in app/login. Each user only ever sees their own
 * row — enforced both in that API route and by Postgres row level
 * security as a second layer.
 *
 * Model calls go through this app's own /api/anthropic route, which
 * holds the real GEMINI_API_KEY server-side — never call the model
 * API with a key from the browser.
 */
...
import {
  ...
  ChevronDown, Sliders, Inbox, Bot, Send, Download, FileUp, LogOut,
} from "lucide-react";
import { createClient as createSupabaseBrowserClient } from "../lib/supabase/client";

/* ------------------------------------------------------------------
   Storage shim — same {key, value} shape the rest of the app expects,
   now backed by /api/profile (Supabase) instead of localStorage.
   ------------------------------------------------------------------ */
const KEY_TO_FIELD = {
  "career-profile": "profile",
  "career-applications": "applications",
  "career-edit-signals": "editSignals",
};
...
const storage = {
  async get(key) {
    ...
    try {
      ...
      if (field === "profile" && !raw) return null;
      return { key, value: JSON.stringify(raw ?? (field === "profile" ? null : [])) };
    } catch (e) {
      return null;
    }
  },
  async set(key, value) {
    ...
    try {
      const res = await fetch("/api/profile", {
        ...
        body: JSON.stringify({ [field]: JSON.parse(value) }),
      });
      if (!res.ok) return null;
      return { key, value };
    } catch (e) {
  ...
  async delete(key) {
    ...
    try {
      const res = await fetch("/api/profile", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ [field]: field === "profile" ? null : [] }),
      });
      if (!res.ok) return null;
...
function mergeImportedProfile(current, imported) {
  // Non-destructive on purpose: appends new repeating-list items rather
  // than replacing, and only fills personalInfo fields that are still
  // empty, so importing a CV never overwrites what's already there.
  const withDefaults = (items, empty) => (items || []).map((item) => ({ ...empty, ...item, id: makeId() }));
  const mergedPersonalInfo = { ...current.personalInfo };
  Object.entries(imported.personalInfo || {}).forEach(([key, value]) => {
    if (value && !mergedPersonalInfo[key]) mergedPersonalInfo[key] = value;
  });
  return {
    ...current,
    personalInfo: mergedPersonalInfo,
    skills: [...current.skills, ...withDefaults((imported.skills || []).map((s) => ({ ...s, confidenceScore: 70 })), EMPTY_SKILL)],
    workExperience: [...current.workExperience, ...withDefaults(imported.workExperience, EMPTY_WORK)],
    projects: [...current.projects, ...withDefaults(imported.projects, EMPTY_PROJECT)],
    education: [...current.education, ...withDefaults(imported.education, EMPTY_EDU)],
    certifications: [...current.certifications, ...withDefaults(imported.certifications, EMPTY_CERT)],
  };
}
...
/* ------------------------------------------------------------------
   Import from an existing CV — bootstraps profile fields
   ------------------------------------------------------------------ */
function ImportCvSection({ onImport }) {
  ...
  return (
    <details className="cc-accordion" style={{ marginBottom: "18px" }}>
      <summary className="cc-accordion-summary">
        <span className="cc-accordion-summary-left">
          <FileUp size={16} />
          <span>Import from an existing CV</span>
        </span>
        <ChevronDown size={16} className="cc-accordion-chevron" />
      </summary>
      <div className="cc-accordion-body">
        <div className="cc-accordion-desc">
          Paste your current resume text below to bootstrap the sections below instead of typing everything by hand. This appends to what's already there rather than replacing it, and only fills personal-info fields that are still empty — review each section afterward. Nothing saves until you hit Save Profile.
        </div>
        <Field label="Resume text">
          <textarea className="cc-textarea" style={{ minHeight: "160px" }} value={cvText} onChange={(e) => setCvText(e.target.value)} placeholder="Paste the full text of your current resume here..." />
        </Field>
        <button type="button" className="cc-btn cc-btn-primary" disabled={cvText.trim().length < 50 || status === "loading"} onClick={handleImport}>
          {status === "loading" ? <Loader2 size={15} className="cc-spin" /> : <FileUp size={15} />}
          {status === "loading" ? "Reading..." : "Extract & Add to Profile"}
        </button>
        {status === "error" && <div className="cc-error"><AlertCircle size={16} /><span>{error}</span></div>}
        {summary && (
          <div className="cc-field-hint" style={{ marginTop: "10px" }}>
            Added {summary.workExperience} work {summary.workExperience === 1 ? "entry" : "entries"}, {summary.projects} {summary.projects === 1 ? "project" : "projects"}, {summary.education} education {summary.education === 1 ? "entry" : "entries"}, {summary.certifications} {summary.certifications === 1 ? "certification" : "certifications"}, and {summary.skills} {summary.skills === 1 ? "skill" : "skills"} to the sections below — expand each to review.
          </div>
        )}
      </div>
    </details>
...
/* ------------------------------------------------------------------
   ...
   ------------------------------------------------------------------ */
function ProfilePage({ profile, updateSection, updateInterestsText, onImportCv }) {
  return (
    <div>
      ...
      <ImportCvSection onImport={onImportCv} />
...
function extractJson(text) {
  ...
  try {
    ...
    // Model sometimes adds a stray sentence before/after the JSON despite
...
- HARD LIMITS, not suggestions — count as you write, and stop at the limit even mid-thought: summary ≤ 2 sentences. Experience: ≤ 3 entries, ≤ 3 bullets each, ≤ 20 words per bullet. Projects: ≤ 2 entries, ≤ 2 bullets each. Cover letter: ≤ 180 words total. matchedKeywords: ≤ 8 items. missingKeywords: ≤ 6 items. A longer, more detailed job description does NOT justify exceeding these — summarize more aggressively instead of writing more.
- Also assess ATS keyword alignment: identify the concrete skills, tools, and requirements stated or implied in the job description, then compare against what's actually in the profile. Return a 0-100 score estimating overlap, matchedKeywords genuinely present in the profile, and missingKeywords that appear in the job description but aren't in the profile. Never suggest inventing or padding the profile to close a gap — missing means missing, and the resume itself must still only use real profile content.
${styleContext ? "\n" + styleContext + "\n" : ""}
JSON shape (respond with exactly this structure and nothing else):
...
Your entire response must be that one JSON object — the first character must be { and the last character must be }. No markdown fences, no preamble, no explanation before or after it.`;
}
...
async function generateApplication(profile, jobDescription, preferences, editSignals) {
  ...
  const trimmedJd = jobDescription.length > JD_MAX_CHARS
    ? jobDescription.slice(0, JD_MAX_CHARS) + "\n[...truncated — job description was longer than this]"
    : jobDescription;
  ...
  try {
    ...
    throw new Error("Got a response back but couldn't parse it. Try again — if it keeps happening, try a shorter job description.");
  }
}
...
const CV_EXTRACT_SYSTEM_PROMPT = `You extract structured career data from resume text. Read the resume and return ONLY a single valid JSON object — no markdown fences, no commentary — matching exactly this shape:
{"personalInfo":{"name":"","email":"","phone":"","location":"","linkedin":"","github":"","portfolio":"","personalWebsite":""},"skills":[{"name":"","category":"","level":"","yearsExperience":0,"lastUsed":""}],"workExperience":[{"company":"","position":"","location":"","startDate":"","endDate":"","current":false,"responsibilitiesText":"","technologiesText":"","achievementsText":""}],"projects":[{"title":"","description":"","technologiesText":"","githubLink":"","demoLink":"","problemsSolved":"","tagsText":""}],"education":[{"school":"","degree":"","gpa":"","graduationYear":""}],"certifications":[{"name":"","provider":"","dateEarned":"","expiryDate":"","credentialId":"","verificationUrl":""}]}

Rules:
- Only include information explicitly present in the resume. Never invent or infer anything that isn't there.
- level: one of "beginner", "intermediate", "advanced", "expert" — infer from how the skill is described; default to "intermediate" if unclear.
- responsibilitiesText and achievementsText: newline-separated, one bullet per line, taken from the resume's own bullet points.
- technologiesText and tagsText: comma-separated.
- Missing sections: return an empty array. Missing personal info fields: return an empty string.`;
...
async function extractProfileFromCv(cvText) {
  ...
  const data = await response.json();
  if (!response.ok) throw new Error(data?.error || "Couldn't read that resume right now.");
  const text = (data.content || []).map((block) => (block.type === "text" ? block.text : "")).join("\n");
  try {
    return extractJson(text);
  } catch (e) {
    throw new Error("Got a response back but couldn't parse it. Try again, or paste it as plain text instead of a copy-pasted PDF layout.");
  }
}
...
function GenerateSection({ profile, preferences, editSignals, readiness, onAddEditSignal, onLogApplication }) {
  ...
  return (
    <div>
      ...
      <Field label="Job description" hint={jobDescription.length > 4000 ? `${jobDescription.length} characters — only the first 4,000 will be used` : undefined}>
...
/* ------------------------------------------------------------------
   App
   ------------------------------------------------------------------ */
function SignOutButton({ userEmail }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
      ...
      <button type="button" className="cc-btn cc-btn-ghost cc-btn-sm" onClick={handleSignOut}>
...
export default function App({ userEmail }) {
  ...
  const handleImportCv = (extracted) => setProfile((p) => mergeImportedProfile(p, extracted));
        ...
        <>
          ...
          <div className="cc-shell">
            ...
            <main className="cc-main">
              {activeTab === "profile" && <ProfilePage profile={profile} updateSection={updateSection} updateInterestsText={updateInterestsText} onImportCv={handleImportCv} />}
