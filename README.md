...
After `npm install` you'll likely see 3 high-severity warnings for `postcss`
and `sharp`. Both are internal dependencies of Next.js itself (used for CSS
processing and image optimization), not something this project depends on
directly, and this app doesn't use image optimization at all. The fix `npm`
suggests (`npm audit fix --force`) would downgrade Next.js to version 9 —
